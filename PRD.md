...
| 화학수축 진단 @ 28 d | 0.03–0.08 mL/g(반응 시멘트) 자릿수. **rev.2 원인 규명(2026-07-17)**: 초과분(실측 0.10~0.13)은 C-S-H 겔공극률 기본값 0.28이 결정학적 골격 몰부피 대비 과소해 벌크가 공간을 덜 채운 것 — 수축 앵커(0.06 mL/g)로 역산한 캘리브레이션 값은 **ε*(CNASH)=0.384, ε*(CSHQ)=0.336** (전자는 CEMHYD3D의 C-S-H 겔공극률 ~0.38과 일치). 기본값은 해시·재현성 계약 때문에 0.28 유지 — 캘리브레이션 런은 `gems_gel_porosity`를 config에 **명시**해 사용한다 |

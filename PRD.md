...
- 시계열 요약(JSON): 상별 alpha, 상별 mol, 물 분배(자유/겔/결합), 총·모세관 공극률, 화학수축 진단,
  ...
  v3.0/E2: 클러스터 해상도 조성 관측치 `cluster_ca_si` — 클러스터별 규산 고용체
  Ca/Si(자기 풀 유래, 더스트 풀 생략)와 report의 중앙 슬라이스 Ca/Si 지도
  PNG(`casi_ckpt_*.png`, 청→적) — SEM-EDS 원소 지도에 대응하는 공간 조성 산출물.
...
0 = 미보유; 그 외는 손상으로 거부)·동일한 액체 겹침 리매핑을 따른다. 양(amount)이
아니라 **비율만** 쓰는 필드다: 급이량은 기존 부피 비례 그대로, 조성만 자기 풀에서
가져온다(풀이 더스트/부재면 글로벌 채널 비율 fallback = E1 거동). 해결(solved)된
클러스터의 풀은 그 스텝 자신의 파슬로 **절대 교체**(누적 아님 — 자기치유),
동결 클러스터는 유지, 겹침 0 행은 소멸(보존은 글로벌 원장이 지므로 dryout 아님).
외부 v3 체크포인트가 없어 FORMAT_VERSION 3에 동승(재단절 없음).
...
- **소유권(Scheme S)**: 매 스텝 recon 파티션(자체 액체 라벨, 없으면 최다 접촉 이웃 클러스터)이
  ...
  자신의 endmember 풀 비율에서 오고(§2.3), 급이 원소는 그 endmember × DCH 원소행으로
  유도되어 endmember 폐합 1b(b)의 급이변이 구성상 정확해진다. 글로벌 평균 조성은
  풀이 없는 클러스터(신생/재습윤)의 fallback으로만 남는다 — E1의 문서화된 근사가
  "풀 있는 곳에서는 제거"로 승격.

...
- [ ] **RT-S1d (설계 확정, 미구현)** — S1-OPEN-2 황산염 트랩 아티팩트
      수정: S 단계 부분계에 소유 CH 완충 평형상 포함(PRD 4.6.5). 실측: 재제공
      부분계 pH 0.72(calibrated)/−0.26(알칼리) → 연산자가 일방향 싱크. S1c
      28d·알칼리 28d 수치는 재해석 대상; 황산염 침식 qualification 블록.

...
- [x] **RT-W4② 1단계 (2026-09-03)** — 32³ NP 스크리닝: t_half 0.0955 h ≙ 스칼라
      D₀ 0.84e-9(W4.2b 청정 앵커 0.8e-9와 일치). 2단계 64³ NP 진행 중(체인은
      v5 워크트리 `TINN_v5`에 고정). rt-cl 병합(FORMAT 6, E=12), 스위트 298.
...
- [x] **RT-R01/D1/D2/R02 (2026-09-08, review-2026-09)** — 적용 플럭스 전하 잔차
      지표+dt 제어(32³ 실측 p50 0.475), 체크포인트 화학 입력 identity 해시·재시작
      검증, 폴백 크기 보고(원소 mol+스칼라 비율), 겔 연결성 즉시 경고(설계안 PRD 4.6.2).
      스위트 304 passed(사전존재 지오메트리 해시 실패 1 무관).
- [x] **RT-R06 + trace-water 스킵 (2026-09-08, review-2026-09)** — S–R 분리 dt 수렴
      실측(28d Cl 0.5M, dt 0.0014/0.0007/0.00035): 결합 Cl spread 0.48%(<2%) 강건, 그러나
      **CH 부피·수착 S는 dt 비수렴**(CH 소비 dt 반감당 ~2배) → S–R 고정점 반복 설계 항목 등록.
      drained trace-water 반응자 수착 실패는 dry처럼 스킵(크로스-플랫폼 수렴 엣지 해결).
